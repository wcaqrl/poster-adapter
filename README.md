...
4. 将这里的文件推送到 `poster-adapter/main`，在 **Actions → Update Poster for LazyCat → Run workflow** 保持默认的 **dry-run** 勾选，验证是否发现最新版本；然后取消勾选再运行一次才会转存、打包和送审。之后每天自动检查。定时任务只在默认分支运行，GitHub 可能延迟调度。
